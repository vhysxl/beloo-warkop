...
export default async function POST(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const { nama, email, telepon, password, provider } = req.body;
        ...
        if (existingUser.rowCount !== null && existingUser.rowCount > 0) {
            if (existingUser.rows[0].provider === 'google') {
                return res.status(400).json({ message: 'Email telah terdaftar menggunakan google, silakan login dengan google' })
            }
